...
+ apartment-user：用户管理 user部分会很重 包含了入住调宿退宿状态跟进 圆圈1 2 3 4 6 8
+ apartment-apartment：公寓-房间-床位管理
...
| login_account_id | bigint       | 逻辑外键，与login_account表中id字段构成一一对应关系。也是管理员对应的手机号。 |
| name             | varchar(15)  | 管理员全名                                                   |
| role             | int          | 管理方面。暂且可分为0入住、调宿管理人员。1财务管理人员。     |
...
| foreman_login_account_id | bigint       | 负责管理该公寓的主要班组长的登录账号id **可null** |
| apartment_name           | varchar(20)  | 公寓名称，eg.望江门公寓                           |
| position                 | varchar(255) | 公寓所在具体地名                                  |
| position_latitude        | double       | 公寓所在地点纬度                                  |
| position_longitude       | double       | 公寓所在地点经度                                  |
| status                   | int          | 公寓状态 0正常 1启用中 2弃用中 3已弃用 default 0  |
