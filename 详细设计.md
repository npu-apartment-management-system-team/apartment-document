...
+ apartment-accommodation：住宿业务管理 包含了入住调宿退宿状态跟进 圆圈1 2 3 4 6 8
...
| payment_id         | bigint       | 逻辑外键，押金订单号，与payment表的id构成映射关系。          |
| type               | int          | 申请类型。包括0入住、1调宿、2退宿                            |
| file_url           | varchar(512) | 申请文件在OSS的存档URL。不可删除，冷备。                     |
| application_status | varchar(10)  | 申请进展，应该是一个两段式的结构。eg.0_1 入住本单位审批中 具体见常量类 考虑撤回 |
